...
class InpiCrawler {
  ...
  async detectFieldsIntelligently(page) {
    ...
    console.log('Groq failed, using enhanced fallback detection');
  ...
  async fallbackDetection(page) {
    const detectedFields = await page.evaluate(() => {
      ...
      inputs.forEach(input => {
        ...
        // Login field
        if (!fields.loginField &&
            (name.includes('login') || name.includes('usuario') || id.includes('login') || id.includes('usuario'))) {
          fields.loginField = input.name || input.id;
        }

        // Password field
        if (!fields.passwordField &&
            (type === 'password' || name.includes('senha') || id.includes('senha'))) {
          fields.passwordField = input.name || input.id;
        }

        // Search field (mais robusto)
        if (!fields.searchField &&
            (name.includes('expressao') || name.includes('palavra') || name.includes('search') || name.includes('busca') ||
             id.includes('expressao') || id.includes('palavra') || id.includes('search') || id.includes('busca') ||
             placeholder.includes('expressao') || placeholder.includes('palavra') || placeholder.includes('search') || placeholder.includes('busca'))) {
          fields.searchField = input.name || input.id;
        }

        // Submit selector
        if (!fields.submitSelector && (type === 'submit' || input.tagName.toLowerCase() === 'button')) {
          fields.submitSelector = `input[type="submit"], button[type="submit"], button`;
        }
      });
    ...
    // Garantir que sempre haja fallback padrão
    const fields = {
    ...
    console.log('Enhanced fallback fields detected:', fields);
    return fields;
  }
  ...
  async performLogin(page, fields) {
    ...
    try {
      ...
      if (!loginInput) throw new Error('Login field not found');
  ...
  async extractResults(page) {
    ...
    const patents = await page.evaluate(() => {
      ...
      rows.forEach(row => {
        ...
        if (cells.length >= 3 && !text.startsWith('Pedido\t') && !text.includes('Login') && text.trim() && cells[0]?.innerText?.trim() !== 'Pedido') {
          results.push({
